...
- 历史 host-action 控制帧只属于迁移兼容数据。runtime 必须在 transcript projection 前消费或拒绝它们，GeeAgentMac 不能把它们当普通聊天文本展示。可见 chat 应按事件顺序展示用户文本、thinking state、tool invocation/result 行，然后展示最终 assistant 回复。
