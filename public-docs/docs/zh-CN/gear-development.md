...
- 如果当前 SDK session 没有暴露 `gee` MCP tools，agent 必须使用通用 `<gee-host-actions>` fallback directive，不能去读源码、绕到 shell 里找产品内部命令，或者声称 bridge 不可用。这个 directive 使用同样三种 bridge operation，并且仍然会暂停 run，交给 GeeAgentMac host 执行。
- `host_action_intents` 也允许一次 runtime turn 返回 native actions，由 GeeAgentMac 按顺序执行。这个快捷路径只适合简单、确定性的第一方 Gear 请求，例如用户要求媒体库只展示视频文件；复杂采集或多 Gear 请求应该走 agent-planned MCP bridge。
- 过渡期内，第一方媒体库的直接请求可以把中英文的视频、图片、全部、星标和 PNG 等扩展名过滤路由到 `media.filter`，而不是进入编码循环。
- 通过 `media.filter` 设置的媒体库筛选会作为 active filters 体现在原生 UI 中。用户可以通过 `All` 或 `Clear filters` 回到完整媒体视图。
- `media.filter`、`media.focus_folder` 和 `media.import_files` 要求媒体库已经获得 macOS 授权。媒体 Gear 应先尝试恢复已保存的 security-scoped access；如果授权缺失或失效，必须返回带有 `code: "gear.media.authorization_required"` 和 `media.library` 的 `navigate.module` 意图的结构化失败，而不是误报成功。
...
当前 SDK fallback directive 调用形态：
...
Fallback directive 规则：

- 允许的 `tool_id` 只有 `gee.app.openSurface`、`gee.gear.listCapabilities` 和 `gee.gear.invoke`。
- directive 是统一 Gear bridge 的传输 fallback，不是针对某个 Gear 的特例 workaround。
- agent 应输出一个有边界的 directive，等待结构化 host results，再根据结果继续规划。
- 最终展示给用户的自然语言回复仍由 active agent/LLM 在执行后生成，不由 directive parser、Gear adapter 或 local router 硬编码。
- 如果 App 重启后仍残留 pending host actions，必须用结构化失败结果让它们过期，不能自动重放。这样可以避免下载、抓取、导入、写书签等副作用动作被重复执行。
...
- Agent capabilities 包括 `media.filter`、`media.focus_folder` 和 `media.import_files`。`media.import_files` 把本地媒体路径导入已授权的媒体库，能在可能时恢复已保存的访问授权，返回导入后的 item 路径供多 Gear 工作流继续使用，并在源文件不存在时通过 `missing_paths` 报告。如果授权缺失，它会把可读路径保留为 `pending_paths`，打开 Media Library 界面，并返回结构化失败交给 active agent/LLM 解释。
...
- Gear 接收 URL，嗅探媒体 metadata，下载音频或视频，并提取 transcript text。
...
- Twitter/X status URL 默认需要下载视频，除非用户明确只想保存 metadata 或明确不要下载媒体。需要推文/媒体细节时先用 `twitter.fetch_tweet`，再用 `smartyt.download_now` 执行视频采集。
...
- 当 SDK session 没有暴露 MCP tools 时，提供通用 `<gee-host-actions>` fallback directive。
...
- fallback directive 必须能通过 `gear_id` 和 `capability_id` 支持所有 ready Gear，不能为 Twitter、media、bookmark 或任何单一 Gear 写特例。
