...
- 外部 Codex 调用现在通过生成的 `geeagent-codex` plugin 和 Gee MCP server 创建 shared-store external invocations。GeeAgentMac 会轮询该队列，并把 `gee_invoke_capability` / `gee_open_surface` 通过 runtime 使用的同一 GearHost bridge 执行，然后 Codex 通过 `gee_get_invocation` 读取结果。第一批导出的内置能力是低风险的 `media.generator` 模型/任务查询，以及 `media.library` 视图过滤/文件夹聚焦。
...
- 外部 Codex 覆盖范围仍然有意保持很窄。只有显式设置 `exports.codex.enabled: true` 的 capability 对 Codex 可见；provider 生成、下载、导入、用户文件和高副作用能力会保持隐藏或显式禁用，直到它们的 approval、artifact 和 failure 语义完成端到端审计。
...
当前实现状态：agent runtime 已经有基于 manifest 的 export status/list/describe 命令、面向 Codex 的 MCP stdio server、shared-store external invocation queue，以及本地 `geeagent-codex` plugin generator。当调用方显式传入 `marketplace_path` 时，generator 可以刷新本地 Codex marketplace entry；默认不会写 marketplace state。`gee_status`、`gee_list_capabilities` 和 `gee_describe_capability` 由 manifest projection 提供。`gee_invoke_capability` 和 `gee_open_surface` 会创建 external invocation，由 GeeAgentMac 通过 live GearHost bridge 执行；`gee_get_invocation` 返回已记录的 status/result。如果 GeeAgentMac 没有运行或无法清空队列，Codex 会收到带 `fallback_attempted: false` 的 pending/failed/blocked/degraded 结构化结果；MCP server 不会执行 Gear 业务逻辑，也不会运行 fallback scripts。
...
生成的 plugin package 包含 `.codex-plugin/plugin.json`、`.mcp.json` 和 `skills/gee-capabilities/SKILL.md`。`.mcp.json` 会让 Codex 调用 `native-runtime codex-mcp`；已安装 Gear 的可用性仍然来自 MCP server 的实时数据，而不是静态 plugin metadata。
...
- Gear 优先支持图片生成，走全局 Xenodia 渠道。当前启用模型是 `nano-banana-pro` 和 `gpt-image-2`。
...
- Task state 写入 `~/Library/Application Support/GeeAgent/gear-data/media.generator/tasks/<task-id>/task.json`。
...
- Agent capabilities 是 `media_generator.list_models`、`media_generator.create_task` 和 `media_generator.get_task`。每个 capability 返回结构化 task/model 数据，由 active agent/LLM 生成最终回复。
