...
- 完整 Codex plugin projection 还没有落地。当前第一段实现已经在 agent-runtime 中提供基于 manifest 的只读导出投影，可用于查询 export status、capability list 和 capability description。Invocation、surface opening、invocation history、生成的 Codex plugin files，以及 Gee MCP export server 仍属于后续计划。
...
当前实现状态：agent runtime 已经有基于 manifest 的只读命令，用于 export status、capability listing 和 capability description。这些命令是未来 Gee MCP export server 会包装的核心逻辑。`gee_invoke_capability`、`gee_open_surface`、`gee_get_invocation` 和生成的 Codex plugin files 仍是 planned，不是已可用能力。
...
Codex 发起的调用是 external invocation。它们应记录 caller metadata、normalized input、structured results、artifact references，以及 failure 或 recovery reasons。它们不会变成隐藏的 GeeAgent chat turn。如果 GeeAgent、GearHost、目标 Gear、provider channel、permission 或 live bridge 不可用，Codex 会收到带真实原因的 structured failed、blocked 或 degraded result。
...
- 来自粘贴参考图和生成结果的最近图片链接写入 `~/Library/Application Support/GeeAgent/gear-data/media.generator/image-history.json`。History sheet 复用这些链接作为参考图，不再打开文件选择器。
