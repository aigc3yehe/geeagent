...
- Legacy host-action control frames は migration-only data です。Runtime は transcript projection の前に consume または reject し、GeeAgentMac は normal chat text として表示してはいけません。Visible chat は user text、thinking state、tool invocation/result rows、final assistant reply を event order で表示します。
