...
- Gear は global Xenodia channel で image generation と video generation を support する。現在 enabled image models は `nano-banana-pro` と `gpt-image-2`。`image-2` は `gpt-image-2` の user-facing alias として受け付ける。現在 enabled video models は `veo3.1`、`veo3.1_fast`、`veo3.1_lite`、`seedance-2`、`seedance-2-fast`。
