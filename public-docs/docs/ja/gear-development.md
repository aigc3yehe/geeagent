...
- full Codex plugin projection はまだ実装されていない。現在の first implementation slice は agent-runtime に manifest-backed read-only projection を提供し、export status、capability listing、capability description を扱う。Invocation、surface opening、invocation history、generated Codex plugin files、Gee MCP export server はまだ planned work である。
...
Current implementation status: agent runtime には manifest-backed read-only commands があり、export status、capability listing、capability description を提供する。これらは future Gee MCP export server が wrap する core logic である。`gee_invoke_capability`、`gee_open_surface`、`gee_get_invocation`、generated Codex plugin files はまだ planned であり、available ではない。
...
Codex-originated calls は external invocations である。caller metadata、normalized input、structured results、artifact references、failure or recovery reasons を記録するべきである。これらは hidden GeeAgent chat turns にはならない。GeeAgent、GearHost、target Gear、provider channel、permission、または live bridge が unavailable の場合、Codex は real reason を持つ structured failed、blocked、または degraded result を受け取る。
...
- Pasted references と generated results からの recent image links は `~/Library/Application Support/GeeAgent/gear-data/media.generator/image-history.json` に保存する。History sheet は file picker を開かず、それらの links を references として reuse する。
