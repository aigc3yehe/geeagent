...
- gear は copy、import、default enable、independent disable/re-enable、update、remove できる。
...
- gear は `gear.json` で name、description、developer、icon/cover image、version、entry、dependencies、permissions、future agent capabilities を宣言する。
...
V1 は実用的であるべきで、過度に複雑にしない。V1 の焦点は local copy/import、bundled gears、dependency preflight と first-run setup、default enablement、user-controlled enablement、native macOS UX、future `gear.invoke` readiness である。policy-level disablement は internal protection state として残り、user の local enable/disable choice とは別である。V1 は remote marketplace、payments、ratings、reviews、remote automatic updates、mandatory developer signing を含まない。
...
- Gears は default enabled である。Gears catalog は per-gear enable/disable control を表示する。Disabled gears は Home widgets を render せず、agent capabilities を expose せず、Gear-owned background services を start しない。
...
- `wechat.channels` は現在の first-party Gear app である。Tencent Yuanbao web session を Keychain に保存し、その session は Gear の native Yuanbao authorization sheet から設定できる。Tencent-owned endpoints で explicit WeChat Channels share URLs または finder-preview URLs を resolve し、short share links はまず official `shortUri` feed route で metadata を取得し、download task records を `gear-data/wechat.channels` に保存し、Tencent が downloadable video URL を返した場合に local artifact として保存する。`wechat_channels.metadata` と `wechat_channels.download` は explicit user-provided WeChat Channels URLs 向けに Codex bridge へ export される。Tencent が request を reject する場合、または metadata だけを返して downloadable `videoUrl` を返さない場合、Gear は `fallback_attempted: false` の structured failed/degraded recovery data を返す。Codex は third-party parser sites、browser scraping、local scripts で代替してはならない。
- `app.icon.forge` は現在の first-party Gear app である。One local source image を、rounded safe-area rendering、`AppIcon.icns`、`AppIcon.iconset`、`AppIcon.appiconset`、1024px preview を含む macOS app icon package、または 780x580 の `assets/icon.png` list tile、`preview-780x580.png`、`gear-icon.json` を含む simple Gear catalog icon package に変換する。
- `todo.manager` は現在の first-party Gear app である。TickTick-style quick capture に着想を得た native local-first task list を提供し、task と list records を `gear-data/todo.manager` に保存し、today/tomorrow/single non-reminder day-part/next-weekday/explicit-date/weekend/reminder の一般的な表現を structured dates に normalize し、authorized の場合は local reminder notifications を schedule し、GeeAgent と Codex bridge calls 用に structured `todo.create`、`todo.query`、`todo.update`、`todo.delete` capabilities を公開する。Create は idempotency keys と short-window exact duplicate reuse を support するため、retry で同じ logical task が重複作成されない。Reminder-only tasks can appear in Today/Upcoming, query payloads include schedule summaries for due/start/reminder-only/all-day/timed/overdue/unscheduled states, and the native inspector can add, edit, remove, and clear absolute or due-relative reminders while showing notification authorization state。Repeat rules は future expansion 用の metadata として保存されるが、generated task instances へはまだ展開されない。V1 には TickTick cloud sync は含まれない。
- `power.video.manager` は現在の first-party read-only Gear app である。User が AI video `runs` workspace を選択し、`script/script-archive.md` を含む各 folder を独立 project として扱う。Flexible script metadata と strict asset views を並べ、primary character looks、storyboard grids、first frames、shot videos、edit renders、final videos、cost summaries、explicit current selections を表示する。V1 では agent capabilities を公開しない。
- `local.project.manager` は現在の first-party Gear app である。Local project launch commands を `gear-data/local.project.manager` に保存し、各 project を native start/stop tile として表示し、tile 全体を primary start/stop target として扱う。User-saved shell command で service を start し、matched local service processes を stop する。Status は reachable configured frontend port、Gear-managed launch PID、command fingerprint の順に確認する。Running project に configured frontend port がある場合は Open Page control を表示する。Project tile の remove は Gear configuration だけを削除し、project files は削除しない。Process start/stop は high-side-effect local work であるため、V1 では agent capabilities を無効にする。
- `telegram.bridge` は現在の first-party Gear である。Native Telegram Bridge surface、GeeAgent local app data に token を保存する GearHost-backed push-only Telegram delivery、GeeAgent direct chat と Codex Remote commands 用の main-window-safe delayed app-started polling service、Gee direct messages の runtime channel ingress を提供する。Setup surface は two-way conversation bots と one-way push channels を分離し、初回設定で required な fields を明示し、入力された bot token から latest Telegram update chat ID を取得して push target field に入力でき、local conversation history、runtime channel ingress history、または Bot API updates から latest Telegram sender user ID を Gee/Codex conversation bot allowlist に入力でき、bridge の inbound/outbound threads を表示する separate Telegram Conversations view を備える。Gee Direct は `/new` による bound runtime conversation reset を support し、その reset acknowledgement を reset command に限定して normal reply として再利用せず、contextual native GearHost execution により validated local files を active Telegram chat へ送信でき、runtime failure は local record のみにせず同じ Telegram chat へ `runtime_failed` として返す。Push-only channels は text、images、videos、GIFs、その他の readable local files を configured outbound Telegram targets へ送信できる。Inbound Telegram media attachments は remote Telegram artifact references として runtime ingress と replay に記録される。Unsupported または unreadable な attachment shapes は明示的に失敗し、bridge は media を download せず transcript text に inline 展開しない。Status output は `gateway_health` diagnostics を含み、missing tokens、webhook-not-ready accounts、polling offsets、push-channel counts、lifecycle readiness（`polling_loop`、`poll_interval_ms`、webhook `not_implemented`）を report するが、Telegram へ接続せず fallback routes も試さない。Standalone worker の `poll-loop` も daemon supervision 用に structured `poll_loop_started`、`poll_loop_iteration_failed`、`poll_loop_stopped` events を emit する。Codex Remote は `/start`/`/help`、`/list`/`/recent`、`/tracked`、`/open`、`/latest`、`/desktop`、`/newcodex [prompt]`、`/mycodex`、`/cancel`、`/send <text>`、`/send <session_id> <prompt>` を support し、prompt send は explicit CLI resume mode を使う。Native bridge は Telegram bot command menu を登録し、paginated project selection、paginated thread browsing、Track/Untrack、Open/Latest thread actions に inline keyboard buttons を送信し、`/list` は各 Codex session に記録された workspace/cwd で group 化し、Codex app で見えない internal conversations を Telegram に出さないよう Codex Desktop-originated non-subagent sessions だけを list する。Telegram-created CLI sessions は chat ごとの bounded registry に保存され、`/mycodex` で list される。Open、Latest、Send はそれらの session を対象にできるが、Codex Desktop thread ではないため `/desktop [session_id]` は block する。`/desktop [session_id]` は thread ごとの inline action ではなく slash command として残す。Codex thread を選択したあと plain text または `/send <text>` を送ると、session を即座に resume せず Confirm/Cancel の確認に stage する。Native と worker の file-scan list paths は bounded recent Codex session date directories のみを読み、directory ごとの file 数と total candidates を cap し、JSONL summaries を incremental に読み、`/list` の project summaries を compact に保つため、大きな session stores を full-history walk せず oversized Telegram replies も生成しない。`/latest` は selected session file の bounded tail window だけを読む。Long conversational replies は truncate せず複数の Telegram messages に split する。Long plain-text push messages も split し、Telegram parse mode 付きの long push messages は formatting tags を invalid chunks に分割しないよう block する。Telegram send stalls は structured timeout failures になり、polling offsets は recorded evidence を残して進められる。Unavailable app-server modes は fallback せず structured failure を返す。Codex export は status/list/send push text and file capabilities で enabled であり、channel creation と contextual direct file send は bot token binding、target confirmation、active-chat identity が local configuration/runtime steps であるため Gee-native のままにする。
- Telegram-created Codex CLI sessions は app-owned Codex Remote workspace から起動し、Codex の explicit `--skip-git-repo-check` option を使う。Telegram がそれらの session を resume するときも同じ non-repo semantics を保ち、失敗は fallback execution ではなく structured Codex Remote failure として表面化する。
- Codex Remote inline button actions は Telegram callback acknowledgement の完了を待たずに選択された command を実行するため、`answerCallbackQuery` request が stall しても Start、Send、Cancel、paging、Open、Latest actions は block されない。
- Native app control は GeeAgent の shared host capability であり、App Chat/runtime には `native_app_control`（`gee.nativeApp.control`）として公開される。V1 は Codex Desktop の `app_id: "codex"` と `action: "new_chat_and_send"` を support する。この path は Mac 上の Codex Desktop を開き、新しい chat を作成し、user-provided prompt をそのまま paste して自動送信する。Codex.app、macOS Accessibility permission、New Chat、prompt input、Send に到達できない場合は structured blocked/failed state を返す。Telegram Gee Direct messages は同じ runtime path に入り、active run からこの capability を使える。Codex Remote は native app control を own せず、execute しない。Gee Direct runtime が空 reply を返した場合は、polling offset を silently advance せず `runtime_empty_reply` を返す。
- Telegram inbound polling は GeeAgentMac app が owner である。GeeAgentMac は inbound polling service を開始する前に local single-owner lock を取得する。Legacy standalone `tg-codex` launchd worker は app-owned bridge と並行して実行しない。Native Codex Remote の `/list` scan は main actor の外で実行され、bounded recent session discovery と short cache を使うため、大きな Codex store でも workbench を freeze しない。
- Native Codex Remote は reply work の前に polling offset を進め、`/list` scan は request time、count、byte volume、workspace/cwd metadata、persisted session index の上限を持ち、Telegram-created CLI session recall は chat ごとの registry で bounded にし、`/send` と `/newcodex` が返す Codex CLI command output も bounded にするため、crash や Telegram send stall の後に同じ重い update を再処理しない。Explicit `/list` と `/tracked` commands は bounded session index を refresh し、pagination と action callbacks は remembered index を reuse して button numbers を stable に保つ。Telegram flow は layered retrieval を使う。`/list` は Desktop project pages、`/project` は bounded Desktop conversation-title pages、`/mycodex` は bounded Telegram-created CLI session pages、`/latest` は selected thread の latest bounded public assistant reply だけを返す。GeeAgentMac は Codex-originated external invocations を poll するとき、app idle 中に full runtime snapshots を繰り返し起動するのではなく、lightweight shared-store projection を読む。
...
- external Codex calls は generated `geeagent-codex` plugin と Gee MCP server により shared-store external invocations を作成する。GeeAgentMac はその queue を poll し、`gee_invoke_capability` / `gee_open_surface` を runtime と同じ GearHost bridge で drain し、Codex は `gee_get_invocation` で recorded results を読む。Read-only な `media.generator` model/task queries は優先処理され、短い wait window を使う。Exported built-in capabilities には low-risk な `media.generator` model/task queries、explicit user request 専用の high-risk `media.generator/media_generator.create_task` image task、image batch、または video task creation、`media.library` view filtering/folder focus と explicit local file import、explicit user request 専用の high-risk `app.icon.forge/app_icon.generate` app icon package generation、explicit user request 専用の high-risk `app.icon.forge/gear_icon.generate` Gear catalog icon generation、medium-risk な `bookmark.vault/bookmark.save` の Gear-owned bookmark writes、medium-risk `smartyt.media/smartyt.search_candidates` media search including Douyin and Xiaohongshu platforms、high-risk `smartyt.media/smartyt.download_now` explicit media download、medium-risk explicit URL の `wespy.reader/wespy.fetch_article` と `wespy.reader/wespy.list_album`、high-risk bounded `wespy.reader/wespy.fetch_album`、medium-risk な `wechat.watcher/wechat.latest_articles` for explicit WeChat public-account names、medium-risk `wechat.channels/wechat_channels.metadata` と high-risk `wechat.channels/wechat_channels.download` for explicit WeChat Channels URLs、low-risk な `todo.manager/todo.query`、medium-risk な `todo.manager/todo.create` と `todo.manager/todo.update`、high-risk soft-delete `todo.manager/todo.delete`、medium-risk な `telegram.bridge/telegram_push.send_message`、そして high-risk explicit-file `telegram.bridge/telegram_push.send_file` configured push-only Telegram channel delivery が含まれる。Contextual `telegram_direct.send_file` は detached Codex direct-chat calls とは分離される。
...
- `icon` は現在 Gears catalog / list surface で使われる。
- Simple spec は `gee.gear.icon.v1`：`assets/icon.png` に 780x580 PNG tile を置き、manifest で `"icon": "assets/icon.png"` と参照する。
- icon file が missing の場合、catalog は `cover`、次に built-in placeholder へ fallback する。
- `cover` はより幅広い card image が必要な surfaces のための optional visual art のままである。
...
- `Enable` / `Disable`：user の local gear enablement を切り替える。Disabled gears は installed のままだが、Home widgets を render せず、agent capabilities を expose せず、Gear-owned background services を start しない。Unavailable gears は引き続き blocked、installing、failed state として別に表示する。
...
- Gear は Xenodia image または video generation のような global network capability が必要であることを宣言できますが、configured channel は Gee host/runtime boundary を通じて request しなければなりません。
- Xenodia media generation は chat routing と同じ infrastructure layer の global provider channel として公開されます。現在の channel には image generation、video generation、task retrieval endpoints があり、将来 dedicated storage upload endpoint を追加できます。
- Generation reference inputs should use the global Xenodia media channel。Separate Xenodia storage API が設定されていない場合、local reference images は Xenodia image generation request の multipart `image_input` として直接送れます。Video references は public URLs または Xenodia `asset://` IDs を使うべきです。Gears must not reintroduce Qiniu or another package-local object store。
...
- shared runtime foundation は active agent に対して、`gee` MCP bridge tools の `app_open_surface`、`native_app_control`、`gear_list_capabilities`、`gear_invoke` としてこれらの controls を公開します。
...
Current implementation status: agent runtime には manifest-backed export status/list/describe commands、Codex-facing MCP stdio server、shared-store external invocation queue、local `geeagent-codex` plugin generator、home-local install command がある。Generated `gee-capabilities` skill は Codex の first entry point であり、GeeAgent plugin とは何かを説明し、generated capability index と per-Gear reference files に誘導し、invocation 前に live MCP discovery / describe を要求する。Install command は default で `~/plugins/geeagent-codex` を書き込み、`~/.agents/plugins/marketplace.json` と Codex が実際に load する cache package `~/.codex/plugins/cache/geeagent-local/geeagent-codex/<plugin-version>` を refresh する。Cache package は home-local plugin package から derive され、development checkout から derive されない。Plugin package は `runtime/native-runtime/<plugin-version>/index.mjs` に versioned built native runtime bundle と exported capabilities 用の generated `gears/<gear-id>/gear.json` manifest projections を含み、generated `.mcp.json` はその plugin-local bundle を指すため、Codex は development checkout や `tsx` loader に依存せず任意の project cwd から bridge を起動できる。`gee_status`、`gee_list_capabilities`、`gee_describe_capability` は manifest projection によって提供される。`gee_invoke_capability` と `gee_open_surface` は external invocation を作成し、GeeAgentMac が live GearHost bridge 経由で drain する。`gee_get_invocation` は recorded status/result を直接読み取り、`wait_ms` で短く terminal state を待つこともできる。Shared-store writes は Codex と GeeAgentMac bridge の concurrent updates の間も queued external invocation records を保持する。GeeAgentMac は read-only な `media.generator` model と task status query を優先して処理するが、結果は同じ external invocation record で完了する。`media.generator/media_generator.create_task` は explicit user-requested image or video generation 用に export され、両 category で `batch_count` 1-4 fan-out を support し、同じ queue から task または batch status/artifact references を返す。`app.icon.forge/app_icon.generate` と `app.icon.forge/gear_icon.generate` は explicit user-requested icon generation 用に export され、GearHost を通じて structured file artifacts を返す。`smartyt.media/smartyt.search_candidates` と `smartyt.media/smartyt.download_now` は explicit media search and selected media download 用に export される。Xiaohongshu/RED search は browser cookies と `xiaohongshu-cli` を使い、Douyin search は Douyin account と required proxy/network route が configured の場合に `dy-cli` を使う。Missing CLI dependencies、missing Douyin login state、または missing Douyin proxy/network readiness は structured failures を返す。`wespy.reader/wespy.fetch_article`、`wespy.reader/wespy.list_album`、`wespy.reader/wespy.fetch_album` は explicit article or album URLs 用に export され、WeSpy task paths、generated files、article metadata、structured failures を GearHost 経由で返す。`wechat.channels/wechat_channels.metadata` と `wechat.channels/wechat_channels.download` は explicit WeChat Channels URLs 用に export され、Tencent profile data、task state、local output paths、structured failed/degraded recovery data を GearHost 経由で返す。`telegram.bridge/telegram_push.send_message` と `telegram.bridge/telegram_push.send_file` は configured push-only Telegram channels 用に export され、real Telegram delivery metadata または structured failed/degraded state を返す。`telegram.bridge/telegram_direct.send_file` は active Gee Direct Telegram chat に依存するため native runtime-only のままである。GeeAgentMac が running でない、または queue を drain できない場合、Codex は `fallback_attempted: false` を含む pending/failed/blocked/degraded structured result を受け取る。Stale `running` invocation は degraded になり、automatic retry ではなく manual-retry recovery を返す。MCP server は Gear business logic を実行せず、fallback scripts も実行しない。
...
Generated plugin package には `.codex-plugin/plugin.json`、`.mcp.json`、versioned native runtime bundle、`gears/<gear-id>/gear.json` manifest projections、`skills/gee-capabilities/SKILL.md`、`references/capability-index.md`、generated per-Gear reference files が含まれる。References と manifest projections は offline orientation and discovery snapshots であり、Gear business logic ではない。`.mcp.json` は Codex を plugin-local の `native-runtime codex-mcp` bundle に接続し、live execution は static plugin metadata ではなく MCP server と GearHost から提供され続ける。
...
- Agent capabilities include `media.filter`, `media.focus_folder`, `media.import_files`, `media.inspect_assets`, `media.update_review_state`, and `media.search_assets`。Current user が local files の import を明示した場合に限り、`media.import_files` は GeeAgent root-agent conversations と Codex conversations の両方から trigger できる。Downloaded or generated media は automatic import しない。Import は optional project、run、source、beat、intended-use、review、flag、note、tag metadata を受け取り、それらを Eagle-compatible item metadata に記録し、saved access を可能な場合に restore し、multi-Gear workflows に item proof を返す。New files は `imported_items` に入り、duplicate files は idempotent success として `action: "import_noop"`、`existing_items`、`duplicate_paths` を返す。Requested source files が見つからない場合は `missing_paths`、unsupported files は `unsupported_paths` で報告する。`media.inspect_assets` は file existence、playability、duration、dimensions、orientation などの technical facts だけを報告する。`media.update_review_state` は human または agent が明示した review status、flags、notes を保存するだけで、Gee は copyright、watermark、likeness、style、beat suitability を判断しない。Supported imported or existing media item が 1 件もない場合、`imported_count: 0` の成功として扱わず、`code: "gear.media.no_supported_files"` の structured failure を返す。Authorization が missing の場合、readable paths を `pending_paths` として保持し、Media Library surface を開き、active agent/LLM が説明できる structured failure を返す。
...
- Gear は global Xenodia channel で image generation と video generation を support する。現在 enabled image models は `nano-banana-pro` と `gpt-image-2`。`image-2` は `gpt-image-2` の user-facing alias として受け付ける。現在 enabled video models は `veo3.1`、`veo3.1_fast`、`veo3.1_lite`、`veo3.1-4k-t`、`veo3.1-components-4k-t`、`veo3.1-pro-4k-t`、`veo3.1-pro-t`、`veo3.1-t`、`veo3.1-components-t`、`veo3-pro-t`、`veo3-t`、`seedance-2`、`seedance-2-fast`。
- Nano Banana Pro exposes `n=1`, `response_format=url`, `aspect_ratio`, `resolution`, `output_format`, and reference images。GPT Image-2 exposes `n=1`, `response_format=url`, `aspect_ratio`, `resolution`, and reference images。Image and video creation は Xenodia task retrieval の task-only path を使う。GPT Image-2 には `output_format` または `nsfw_checker` を送らない。
- `media_generator.list_models` は各 model の top-level supported values 全体と、ratio-specific resolution constraints を示す `resolution_by_aspect_ratio` map を返す。そのため GPT Image-2 は `1K`、`2K`、`4K` を advertise しつつ、`auto` は `1K` のみ受け付けることも明示する。
- Multi-result generation は image と video tasks の両方で Gear-level `batch_count` 1-4 を使う。Gee は requested result ごとに persisted Xenodia child task を作成し、同じ `batch_id` で group 化し、native task list では one batch row と result grid として表示する。
- Reference image limits are model-specific: Nano Banana Pro は up to 8 total inputs、GPT Image-2 は up to 16 total inputs、Veo は up to 3 image references、Seedance は up to 9 image references。Local references must be JPEG, PNG, or WebP and 30MB or smaller。
- Veo3.1 video は Xenodia `/v1/videos/generations` を使う。`REFERENCE_2_VIDEO` は `veo3.1_fast` が必要であり、text-to-video は image references を拒否し、first/last frame mode は 1-2 image URLs、reference mode は 1-3 image URLs を受け付ける。Veo `-t` alternate-route models は同じ endpoint を使うが、別の payload shape を使う。Public http(s) `imageUrls` のみ、最大 3 references、`aspect_ratio` は `16:9` または `9:16`、`resolution` は `720p`、`1080p`、`4k`、optional fixed `duration=8`、optional boolean `watermark` を support し、legacy Veo の `generationType`、`seeds`、translation、fallback、callback fields は送信しない。Seedance 2.0 video は同じ endpoint を使い、4-15 秒 duration、first/last frame URLs、multimodal reference URL arrays、optional audio generation、web search、NSFW screening を support する。Audio generation は structured unsupported-category placeholder のままである。
- Reference image upload must not use Qiniu from the reference project。Image tasks は local references を Xenodia image request で送信し、video tasks は configured global Xenodia `storage_upload_url` で local references を upload してから、returned public URL または `asset://` ID を video request に渡す。Upload configuration が missing の場合は local paths に fallback せず structured failure にする。
- Task state belongs in `~/Library/Application Support/GeeAgent/gear-data/media.generator/tasks/<task-id>/task.json`。Batch rows は child task records の projection であり、task storage path を置き換えない。Gear は task directories の横に `tasks-index.json` を保持するため、native history は newest page から開き、older tasks を必要に応じて incremental に load でき、refresh のたびに full history を decode しない。Current batch task records use schema version 2；this schema cutover may clear older task history。
...
- Pasted URLs、clipboard images、local files からの recent reference images は `~/Library/Application Support/GeeAgent/gear-data/media.generator/image-history.json` に保存する。History sheet は image mode と video mode の両方で file picker を開かず references を reuse する。Generated results は task records と output caches に残し、reference history へ自動追加しない。
- Task polling は Gear open または history reload 時に `running` / `queued` tasks を resume しなければならない。Polling should treat Xenodia `success` as completed、`fail` as failed、and read generated URLs from the normalized `result` payload。
- Xenodia image と video generation は long-running task として扱う。create、multipart image upload、video create、task-status requests は minimum 30-minute timeout floor を使う。Status request が timed out した場合、provider が still generating の間に failed とせず、local task は `running` のまま polling を続ける。
...
- Native task workbench は status filters、model filters、search、starred results、local-cache badges、Finder reveal、fit-to-window large preview、displayable reference thumbnails の click preview と large reference preview 内の Download action、task list を下へ scroll したときの lower-right floating back-to-top button、generated videos の default-muted hover autoplay と manual sound toggle、URL copy、user-chosen download、task Apply による prompt/model/parameters/references restore、separate reuse-as-reference、task rows からの direct task-history deletion を support する。
- Agent capabilities are `media_generator.list_models`, `media_generator.create_task`, and `media_generator.get_task`。`media_generator.create_task` can be triggered from both GeeAgent root-agent conversations and Codex conversations。Multi-result image and video requests use `batch_count` 1-4 while image provider `n` stays 1。Each returns structured task or batch/model data for the active agent/LLM to summarize。

`smartyt.media`:

- Target: native URL media acquisition gear adapted from the SmartYT reference project。
- Gear は explicit URL または media search query を受け取り、media metadata を sniff し、candidate clips を search し、candidate または URL が明示的に選択された後にだけ audio、video、direct image artifacts を download する。
- V1 uses `yt-dlp` for YouTube candidate search, metadata, downloads, and subtitle files when platform subtitles already exist, and `ffmpeg` / `ffprobe` for media conversion support。Xiaohongshu/RED search は Gear-declared `xiaohongshu-cli` dependency と browser cookies を使う。Douyin search は Douyin account と required proxy/network route が configured の場合に Gear-declared `dy-cli` dependency を使う。Douyin video URLs は同じ `dy-cli` route で download し、Xiaohongshu explicit URLs は SmartYT URL download provider を通る。Speech transcription and voice conversion は current Codex-exported acquisition workflow の対象外である。
- `yt-dlp` backed operations の前に、SmartYT は bounded dependency maintenance を行い、Homebrew の `yt-dlp` upgrade path を 1 日 1 回まで実行して stable version を refresh する。Concurrent SmartYT operations は 1 つの in-process maintenance run を share し、competing Homebrew upgrades を同時に start しない。Maintenance path が unavailable の場合、Gear は problem を隠さず structured failure を返す。YouTube bot verification または cookie requirement は `gear.smartyt.youtube_auth_required` として report され、`yt-dlp` の update だけを recovery とは扱わない。
- Native SmartYT UI は browser-exported Netscape `cookies.txt` file または common browser JSON cookie export を long-term に保存できる。SmartYT は selected file を `~/Library/Application Support/GeeAgent/gear-data/smartyt.media/cookies/yt-dlp-cookies.txt` に normalize し、以後の `yt-dlp` operations では per-command temporary cookie copy を automatic に使う。Agent calls は `cookie_file` を明示して one-call override path を渡すこともできる。Saved cookie source file は extractor cookie-jar writes によって mutate されてはならない。YouTube が saved account cookies の rotation または invalid state を report した場合、SmartYT は active Chrome profile から saved cookie file を refresh し、同じ `yt-dlp` command を 1 回だけ automatic に retry する。UI も manual Chrome refresh action を提供する。
...
- Native SmartYT task UI は queued/running job ごとの progress を表示し、queued または running 状態の job を cancel できる。Completed、failed、cancelled job records は UI から delete でき、delete 時にはその job に紐づく Gear-owned downloaded files、transcript files、task output directory も remove される。Queued または running job は cancel または finish してからでないと delete できない。Structured task payloads は `progress_fraction`、`progress_label`、`can_cancel` を expose し、polling surfaces が同じ state を render できるようにする。
- Codex-exported agent capabilities are `smartyt.search_candidates`, `smartyt.sniff`, `smartyt.download`, `smartyt.download_now`, `smartyt.get_task`, and `smartyt.list_tasks`。`smartyt.search_candidates` は search task を保存し、download せず candidate metadata を返す。`platforms` may include `youtube`, `douyin`, `xiaohongshu`, and aliases such as `xhs` and `rednote`。`smartyt.download` は app UI 向けに queue し、`smartyt.download_now` は artifacts が生成されるまで待って `output_paths` を返す。Douyin provider failures は structured login or proxy/network recovery を返し、Codex-side fallback work は実行しない。Downloaded files は SmartYT task output directory に残り、user が別途 import を明示しない限り Media Library へ automatic import しない。Direct image URLs, including Twitter/X image URLs with an image extension or a `format=` query hint, are treated as image downloads instead of video downloads。The active agent/LLM owns the final user-facing reply。
...
- Target: quick capture、list review、due-date triage、reminders、agent-controlled task updates のための native local-first todo Gear。
...
- Native surface は smart filters と lists の sidebar、compact quick-add row、task list、notification authorization state 付き reminder controls、task inspector を提供する。TickTick task model を参考にするが、V1 では TickTick accounts と sync しない。
- Agent capabilities are `todo.create`, `todo.query`, `todo.update`, and `todo.delete`。They return structured task records、query counts、changed fields、warning codes、storage paths for the active agent/LLM to summarize。
- `todo.create` and `todo.update` can store start dates、due dates、all-day flags、timezone labels、priorities、tags、checklist items、reminder offsets or reminder dates、and an optional repeat rule string。`todo.create` accepts an idempotency key, reuses short-window exact duplicate creates, and can normalize common local-language today、tomorrow、single non-reminder day-part、weekday、explicit date、weekend、and reminder text into structured task fields before saving。Reminder-only tasks are eligible for Today/Upcoming views, and `todo.query` returns schedule summary fields (`date_source`, `date_state`, `time_state`, `is_overdue`, and `relevant_at`) so agents do not need to infer date relevance from raw timestamps。Reminder requests still require a concrete clock time。Repeat rules are preserved as metadata only until recurring-instance expansion is implemented。
- `todo.delete` performs a soft delete by marking the task as deleted and cancelling local notifications。Missing task ids、unknown list ids、invalid dates、reminder requests without a concrete time、unavailable notification authorization、schedule failures must be returned as structured failures or warnings。The Gear must not create implicit fallback lists or report reminder scheduling as complete when macOS did not authorize notifications。
...
- Target: user が明示的に追加した accounts のための native WeChat public-account update watcher。
- Gear は user-provided authenticated WeChat Official Account backend session を必要とする。Session token、cookie、user agent は Keychain に保存し、`gear.json` や Gear data file には保存しない。
- Account search は WeChat の authenticated backend search を使い、backend `fakeid` を watchlist key として記録する。Public WeChat account profile pages は stable watchlist identity として扱わない。
- Watchlist records と check-run history belong in `~/Library/Application Support/GeeAgent/gear-data/wechat.watcher/watcher.json`。
- Agent capabilities are `wechat.auth_status`, `wechat.search_accounts`, `wechat.watch_account`, `wechat.list_watched_accounts`, `wechat.check_updates`, and `wechat.latest_articles`。They return structured status、search results、watched accounts、discovered article URLs、run records、failure codes for the active agent/LLM to summarize。
- `wechat.latest_articles` is exported through the Codex bridge for explicit public-account names。Codex callers must use Gee MCP/GearHost so the live Gear searches the authenticated WeChat backend by name, uses the returned `fakeid`, optionally writes the watchlist entry, and returns article URLs; they must not substitute official websites, search engines, profile-page scraping, or local scripts。
- V1 は personal WeChat follow list を読まず、QR login を行わず、main window 表示前に background polling を開始せず、`wespy.reader` を自動 invoke しない。Multi-Gear workflows may pass discovered article URLs to `wespy.fetch_article` when the user asks for Markdown capture。
- Missing or expired sessions、WeChat API rejection、changed response shape、network failures は `fallback_attempted: false` を含む structured failures として返す必要がある。Gear は別の scraping path に silent switch せず、有効な account session なしに monitoring active と主張してはいけない。

`wechat.channels`:

- Target: explicit user-provided share または finder-preview URLs のための native WeChat Channels downloader。
- Short share links はまず Tencent official `shortUri` feed route で metadata を取得する。Downloadable feed URL の recovery が必要な場合、または public feed route が URL を resolve できない場合だけ configured Tencent Yuanbao web session を使う。Users can configure this session from the Gear's native authorization sheet, which embeds Yuanbao sign-in and saves the resulting browser session inside the Gear boundary。Cookie、user agent、optional current request headers は Keychain に保存し、`gear.json` や Gear task files には保存しない。
- URL support は explicit である: `https://weixin.qq.com/sph/...` と `https://channels.weixin.qq.com/finder-preview/pages/...`。Public-account article URLs は引き続き `wespy.reader`、public-account update monitoring は `wechat.watcher` が担当する。
- Task records belong in `~/Library/Application Support/GeeAgent/gear-data/wechat.channels/tasks/<task-id>/task.json`。Downloaded videos は agent call が explicit `output_dir` を提供しない限り `~/Downloads/WeChat Channels/<task-id>/` に保存される。
- Agent capabilities are `wechat_channels.auth_status`, `wechat_channels.metadata`, `wechat_channels.download`, `wechat_channels.get_task`, and `wechat_channels.list_tasks`。They return structured session status、profile data、task records、local output paths、failed/degraded recovery data for the active agent/LLM to summarize。
- `wechat_channels.metadata` と `wechat_channels.download` は explicit user-provided video URLs 用に Codex bridge へ export される。Codex callers は Gee MCP/GearHost を使い、third-party parser sites、browser scraping、SmartYT、local scripts で代替してはならない。
- Missing required Yuanbao sessions、Tencent API rejection、changed response shape、network failures、resolved profiles without `videoUrl` は `fallback_attempted: false` の structured failures または degraded states として返す必要がある。

`app.icon.forge`:

- Target: macOS app icons と Gear catalog icons のための native icon processing Gear。
- App icon mode は user-selected local image を一つ受け取り、center-crop で square 化し、transparent 1024px canvas の rounded safe-area に描画して complete app icon package を export する。
- App icon artifacts include `<name>.icns`, `<name>.iconset`, `<name>.appiconset`, `preview-1024.png`, and `icon-export.json`。
- Gear icon mode は `gee.gear.icon.v1` package を export する。`assets/icon.png` は Gears catalog / list surface 用の 780x580 PNG tile で、`preview-780x580.png` と `gear-icon.json` も生成する。
...
- Capabilities は GearHost bridge を通じて GeeAgent root-agent conversations と Codex conversations から利用できる。Codex callers は user が明示した local paths だけを渡し、GearHost が返す artifact paths をそのまま報告し、package-local image scripts を実行してはいけない。

`power.video.manager`:

- Target: AI video production `runs` workspaces のための native read-only viewer。
- User が `runs` folder を手動で選択する。Gear は `script/script-archive.md` を直接含む child folder をそれぞれ独立 project として扱い、folders 間で project を merge しない。
- Project list は利用可能な primary character look、title、relative last update、progress labels、cost total を表示し、updated time で sort する。
- Detail surface は script metadata を tolerant に扱い、raw document 表示も残す。一方、asset views は character looks、storyboard grids、first frames、shot videos、edit renders、final videos、diagnostics、current run-state evidence または closeout files が示す explicit selected assets に strict normalize する。
- Timeline view は horizontal で、one row per shot。Discovered first-frame と video versions を file creation/update order で表示する。Video tiles は file から poster thumbnail を表示し、pointer hover で muted looping playback に切り替わる。Review と version reason explanation は行わない。
- Current AI video runs may record accepted first frames、accepted video candidates、route metadata、video costs in `run-state.json`。Older workflow closeout files remain supported when available: `production/current-selection.json` for selected first frames/videos、`editing/current-edit.json` for the selected final/edit render、and `cost-summary.json` for per-project cost。run-state と selection file のどちらも asset を示さない場合、Gear は selected marker を表示しない。
- Agent capabilities are disabled in V1。この Gear は Codex bridge に export されず、gear-specific pseudo-tools も追加してはいけない。

`local.project.manager`:

- Target: local project services を start / stop する native Gear。
- Project records belong in `~/Library/Application Support/GeeAgent/gear-data/local.project.manager/projects.json`。
- 各 project は display name、multiline terminal command、optional frontend port を保存する。Native surface は compact tiles として project を表示し、tile body を primary start/stop control とする。Edit/remove actions、refresh button、status source evidence、Open Page は separate controls として残る。Open Page は project が running で configured frontend port がある場合に表示され、configured localhost URL を開く。
- Running detection は technology-neutral にする。GeeAgent は reachable configured port、Gear-managed PID、saved shell command から得た command fingerprint の順に信頼する。Frontend project では port check が different stacks をまたぐ最も強い signal であり、command matching は confidence の低い process discovery である。
- Stop は現在 resolve された service PIDs を使う。Gear から project を remove しても saved configuration だけを削除し、source directories は触らない。
- Agent capabilities are disabled in V1。Start/stop process control は approval、result、failure semantics review なしに gear-specific pseudo-tool として公開してはいけない。
...
- Douyin and Xiaohongshu/RED search or explicit video URLs should use `smartyt.media`。Keyword search requests call `smartyt.search_candidates` with `platforms` as `douyin` or `xiaohongshu`; Xiaohongshu requires browser cookies and Douyin requires a configured `dy-cli` account plus any proxy/network route reported by the CLI。Selected candidates and explicit URLs download through `smartyt.download_now`。
...
Gear platform extraction sequence は Phase 1 と Phase 2 から始めます。
