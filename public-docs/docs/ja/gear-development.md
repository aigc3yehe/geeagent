...
- Agent capabilities include `media.filter`, `media.focus_folder`, and `media.import_files`。`media.import_files` は local media paths を現在開いている library に import し、multi-Gear workflows が続けて使える imported item paths を返し、requested source files が見つからない場合は `missing_paths` で報告する。
