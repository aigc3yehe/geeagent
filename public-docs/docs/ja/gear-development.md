...
- external Codex calls は generated `geeagent-codex` plugin と Gee MCP server により shared-store external invocations を作成する。GeeAgentMac はその queue を poll し、`gee_invoke_capability` / `gee_open_surface` を runtime と同じ GearHost bridge で drain し、Codex は `gee_get_invocation` で結果を読む。最初に export される built-in capabilities は low-risk な `media.generator` model/task queries と `media.library` view filtering/folder focus である。
...
- external Codex coverage は意図的に狭い。`exports.codex.enabled: true` を明示した capabilities だけが Codex から見える。provider generation、download、import、user-file、高 side-effect capabilities は、approval、artifact、failure semantics が end to end で audit されるまで hidden または explicitly disabled のままにする。
...
Current implementation status: agent runtime には manifest-backed export status/list/describe commands、Codex-facing MCP stdio server、shared-store external invocation queue、local `geeagent-codex` plugin generator がある。Caller が explicit `marketplace_path` を渡した場合、generator は local Codex marketplace entry を refresh できる。Default では marketplace state を書かない。`gee_status`、`gee_list_capabilities`、`gee_describe_capability` は manifest projection によって提供される。`gee_invoke_capability` と `gee_open_surface` は external invocation を作成し、GeeAgentMac が live GearHost bridge 経由で drain する。`gee_get_invocation` は recorded status/result を返す。GeeAgentMac が running でない、または queue を drain できない場合、Codex は `fallback_attempted: false` を含む pending/failed/blocked/degraded structured result を受け取る。MCP server は Gear business logic を実行せず、fallback scripts も実行しない。
...
Generated plugin package には `.codex-plugin/plugin.json`、`.mcp.json`、`skills/gee-capabilities/SKILL.md` が含まれる。`.mcp.json` は Codex を `native-runtime codex-mcp` に接続する。Installed Gear availability は static plugin metadata ではなく、MCP server の live data であり続ける。
...
- Gear は image generation を first に support し、global Xenodia channel を使う。現在 enabled models は `nano-banana-pro` と `gpt-image-2`。
