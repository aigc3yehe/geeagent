...
- Detail surface は script metadata を tolerant に扱い、raw document 表示も残す。一方、asset views は character looks、storyboard grids、first frames、shot videos、edit renders、final videos、diagnostics、closeout files が示す explicit selected assets に strict normalize する。
...
- AI video skills should write closeout files when available: `production/current-selection.json` for selected first frames/videos, `editing/current-edit.json` for the selected final/edit render, and `cost-summary.json` for per-project cost。Selection file がない場合、Gear は selected marker を表示しない。
