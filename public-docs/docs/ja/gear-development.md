...
- `todo.manager` は現在の first-party Gear app である。TickTick-style quick capture に着想を得た native local-first task list を提供し、task と list records を `gear-data/todo.manager` に保存し、authorized の場合は local reminder notifications を schedule し、GeeAgent と Codex bridge calls 用に structured `todo.create`、`todo.query`、`todo.update`、`todo.delete` capabilities を公開する。Repeat rules は future expansion 用の metadata として保存されるが、generated task instances へはまだ展開されない。V1 には TickTick cloud sync は含まれない。
- `telegram.bridge` は現在の first-party Gear である。Native Telegram Bridge surface、GeeAgent local app data に token を保存する GearHost-backed push-only Telegram delivery、GeeAgent direct chat と Codex Remote commands 用の app-started polling service、Gee direct messages の Phase 3 channel ingress を提供する。Setup surface は two-way conversation bots と one-way push channels を分離し、初回設定で required な fields を明示し、入力された bot token から latest Telegram update chat ID を取得して push target field に入力でき、local conversation history、runtime channel ingress history、または Bot API updates から latest Telegram sender user ID を Gee/Codex conversation bot allowlist に入力でき、bridge の inbound/outbound threads を表示する separate Telegram Conversations view を備える。Gee Direct は `/new` による bound runtime conversation reset を support し、contextual native GearHost execution により validated local files を active Telegram chat へ送信でき、Phase 3 runtime failure は local record のみにせず同じ Telegram chat へ `runtime_failed` として返す。Push-only channels は text、images、videos、GIFs、その他の readable local files を configured outbound Telegram targets へ送信できる。Codex Remote は `/start`/`/help`、`/list`/`/recent`、`/tracked`、`/open`、`/latest`、`/desktop`、`/cancel`、`/send <text>`、`/send <session_id> <prompt>` を support し、prompt send は explicit CLI resume mode を使う。Native bridge は Telegram bot command menu を登録し、paginated project selection、paginated thread browsing、Track/Untrack、thread actions に inline keyboard buttons を送信し、`/list` を project root で group 化し、Codex app で見えない internal conversations を Telegram に出さないよう Codex Desktop-originated non-subagent sessions だけを list する。Codex thread を選択したあと plain text を送ると、session を即座に resume せず Confirm/Cancel の確認に stage する。Native file-scan list path は Codex JSONL session summaries を incremental に読み、大きな session files が Telegram replies を block しないようにし、long conversational replies は truncate せず複数の Telegram messages に split する。Long plain-text push messages も split し、Telegram parse mode 付きの long push messages は formatting tags を invalid chunks に分割しないよう block する。Unavailable app-server modes は fallback せず structured failure を返す。Codex export は status/list/send push text and file capabilities で enabled であり、channel creation と contextual direct file send は bot token binding、target confirmation、active-chat identity が local configuration/runtime steps であるため Gee-native のままにする。
...
- `telegram.bridge` は production daemon management と broader setup polish がまだ必要だが、現在の bridge は native GearHost push text and file delivery、local app-data token lookup、push-channel creation、app-started polling、Telegram command menu、inline buttons、project grouping、pagination、Track/Untrack project lists、explicit send confirmation、long conversation/push splitting、Codex Desktop-visible session filtering を備えた Codex remote commands、Phase 3 direct-chat ingress、`/new` conversation reset、Telegram 内 runtime failure reply、allowlist user-ID fetch、contextual Gee Direct local-file sending を含む。
...
- Native surface は smart filters と lists の sidebar、compact quick-add row、task list、task inspector を提供する。TickTick task model を参考にするが、V1 では TickTick accounts と sync しない。
...
- `todo.delete` performs a soft delete by marking the task as deleted and cancelling local notifications。Missing task ids、unknown list ids、invalid dates、unavailable notification authorization、schedule failures must be returned as structured failures or warnings。The Gear must not create implicit fallback lists or report reminder scheduling as complete when macOS did not authorize notifications。
