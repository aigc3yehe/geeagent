...
- external Codex calls は generated `geeagent-codex` plugin と Gee MCP server により shared-store external invocations を作成する。GeeAgentMac はその queue を poll し、`gee_invoke_capability` / `gee_open_surface` を runtime と同じ GearHost bridge で drain し、Codex は `gee_get_invocation` で recorded results を読む。Read-only な `media.generator` model/task queries は優先処理され、短い wait window を使う。Exported built-in capabilities には low-risk な `media.generator` model/task queries、explicit user request 専用の high-risk `media.generator/media_generator.create_task` image task、image batch、または video task creation、`media.library` view filtering/folder focus と explicit local file import、explicit user request 専用の high-risk `app.icon.forge/app_icon.generate` app icon package generation、explicit user request 専用の high-risk `app.icon.forge/gear_icon.generate` Gear catalog icon generation、medium-risk な `bookmark.vault/bookmark.save` の Gear-owned bookmark writes、low-risk な `todo.manager/todo.query`、medium-risk な `todo.manager/todo.create` と `todo.manager/todo.update`、high-risk soft-delete `todo.manager/todo.delete`、medium-risk な `telegram.bridge/telegram_push.send_message`、そして high-risk explicit-file `telegram.bridge/telegram_push.send_file` configured push-only Telegram channel delivery が含まれる。Contextual `telegram_direct.send_file` は detached Codex direct-chat calls とは分離される。
...
- V1 uses `yt-dlp` for candidate search, metadata, downloads, and subtitle files when platform subtitles already exist, and `ffmpeg` / `ffprobe` for media conversion support。Speech transcription and voice conversion は current Codex-exported acquisition workflow の対象外である。
...
- Codex-exported agent capabilities are `smartyt.search_candidates`, `smartyt.sniff`, `smartyt.download`, `smartyt.download_now`, `smartyt.get_task`, and `smartyt.list_tasks`。`smartyt.search_candidates` は search task を保存し、download せず candidate metadata を返す。`smartyt.download` は app UI 向けに queue し、`smartyt.download_now` は artifacts が生成されるまで待って `output_paths` を返す。Downloaded files は SmartYT task output directory に残り、user が別途 import を明示しない限り Media Library へ automatic import しない。Direct image URLs, including Twitter/X image URLs with an image extension or a `format=` query hint, are treated as image downloads instead of video downloads。The active agent/LLM owns the final user-facing reply。
