...
- `todo.manager` is a current first-party Gear app. It provides a native local-first task list inspired by TickTick-style capture, stores task and list records under `gear-data/todo.manager`, schedules local reminder notifications when authorized, and exposes structured `todo.create`, `todo.query`, `todo.update`, and `todo.delete` capabilities for GeeAgent and Codex bridge calls. Repeat rules are stored for future expansion but are not expanded into generated task instances yet; no TickTick cloud sync is included in V1.
- `telegram.bridge` is a current first-party Gear. It provides a native Telegram Bridge surface, GearHost-backed push-only Telegram delivery with tokens stored in GeeAgent local app data, an app-started polling service for GeeAgent direct chat and Codex Remote commands, and Phase 3 channel ingress for Gee direct messages. Its setup surface separates two-way conversation bots from one-way push channels, labels first-setup required fields, can fetch the latest Telegram update chat ID from a provided bot token into the push target field, can fill the latest Telegram sender user ID into a Gee/Codex conversation bot allowlist from local conversation history, runtime channel ingress history, or Bot API updates, and includes a separate Telegram Conversations view for inbound/outbound bridge threads. Gee Direct supports `/new` to reset the bound runtime conversation, can send validated local files back to the active Telegram chat through contextual native GearHost execution, and sends Phase 3 runtime failures back to the same Telegram chat as `runtime_failed` instead of only recording them locally. Push-only channels can send text, images, videos, GIFs, and other readable local files to configured outbound Telegram targets. Codex Remote supports `/start`/`/help`, `/list`/`/recent`, `/tracked`, `/open`, `/latest`, `/desktop`, `/cancel`, `/send <text>`, and `/send <session_id> <prompt>` through explicit CLI resume mode. Its native bridge registers Telegram bot command menus, sends inline keyboard buttons for paginated project selection, paginated thread browsing, Track/Untrack, and thread actions, groups `/list` by project root, and only lists Codex Desktop-originated non-subagent sessions so Telegram does not expose internal conversations that are invisible in the Codex app. Selecting a Codex thread and sending plain text now stages the prompt for explicit Confirm/Cancel instead of immediately resuming the session. The native file-scan list path reads Codex JSONL session summaries incrementally so very large session files do not block Telegram replies, long conversational replies are split into multiple Telegram messages instead of being truncated, and long plain-text push messages are also split while long parse-mode push messages remain blocked to avoid invalid formatting chunks; unavailable app-server modes return structured failure instead of falling back. Codex export is enabled for status/list/send push text and file capabilities; channel creation and contextual direct file send remain Gee-native because bot token binding, target confirmation, and active-chat identity are local configuration/runtime steps.
...
- `telegram.bridge` still needs production daemon management and broader setup polish, but its current bridge already includes native GearHost push text and file delivery, local app-data token lookup, push-channel creation, app-started polling, Codex remote commands with Telegram command menus, inline buttons, project grouping, pagination, Track/Untrack project lists, explicit send confirmation, long conversation/push splitting, Codex Desktop-visible session filtering, Phase 3 direct-chat ingress, `/new` conversation reset, runtime-failure replies in Telegram, allowlist user-ID fetch, and contextual Gee Direct local-file sending.
...
- The native surface provides a sidebar for smart filters and lists, a compact quick-add row, a task list, and a task inspector. It is inspired by TickTick's task model but does not sync with TickTick accounts in V1.
...
- `todo.create` and `todo.update` can store start dates, due dates, all-day flags, timezone labels, priorities, tags, checklist items, reminder offsets or reminder dates, and an optional repeat rule string. Repeat rules are preserved as metadata only until recurring-instance expansion is implemented.
- `todo.delete` performs a soft delete by marking the task as deleted and cancelling local notifications. Missing task ids, unknown list ids, invalid dates, unavailable notification authorization, and schedule failures must be returned as structured failures or warnings. The Gear must not create implicit fallback lists or report reminder scheduling as complete when macOS did not authorize notifications.
