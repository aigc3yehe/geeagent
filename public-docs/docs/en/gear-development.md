...
- External Codex calls now use the generated `geeagent-codex` plugin plus the Gee MCP server to create shared-store external invocations. GeeAgentMac polls that queue and drains `gee_invoke_capability` / `gee_open_surface` through the same GearHost bridge used by the runtime, while Codex reads recorded results with `gee_get_invocation`; read-only `media.generator` model/task queries are prioritized and use short wait windows. The exported built-in capabilities include low-risk `media.generator` model/task queries, high-risk explicit-user `media.generator/media_generator.create_task` image task, image batch, or video task creation, `media.library` view filtering/folder focus plus explicit local file import, high-risk explicit-user `app.icon.forge/app_icon.generate` app icon package generation, high-risk explicit-user `app.icon.forge/gear_icon.generate` Gear catalog icon generation, medium-risk `bookmark.vault/bookmark.save` for Gear-owned bookmark writes, low-risk `todo.manager/todo.query`, medium-risk `todo.manager/todo.create` and `todo.manager/todo.update`, high-risk soft-delete `todo.manager/todo.delete`, medium-risk `telegram.bridge/telegram_push.send_message`, and high-risk explicit-file `telegram.bridge/telegram_push.send_file` for configured push-only Telegram channels. Contextual `telegram_direct.send_file` remains separate from detached Codex direct-chat calls.
...
- Folder management, filtering, starring, Quick Look, Finder handoff, video edit handoff, thumbnail-backed video / gif hover playback, seamless looping previews, and live presentation mode belong to the media gear, not the main workbench.
...
- V1 uses `yt-dlp` for candidate search, metadata, downloads, and subtitle files when platform subtitles already exist, and `ffmpeg` / `ffprobe` for media conversion support. Speech transcription and voice conversion are outside the current Codex-exported acquisition workflow.
...
- Codex-exported agent capabilities are `smartyt.search_candidates`, `smartyt.sniff`, `smartyt.download`, `smartyt.download_now`, `smartyt.get_task`, and `smartyt.list_tasks`. `smartyt.search_candidates` saves a search task and returns candidate metadata without downloading anything. `smartyt.download` queues work for the app UI, while `smartyt.download_now` blocks until artifacts exist and returns `output_paths` for multi-Gear workflows. Downloaded files remain in the SmartYT task output directory and are not imported into Media Library unless the user separately asks for import. Direct image URLs, including Twitter/X image URLs with an image extension or a `format=` query hint, are treated as image downloads instead of video downloads. The active agent/LLM owns the final user-facing reply.
