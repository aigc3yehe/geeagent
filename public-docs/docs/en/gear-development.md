...
- The detail surface keeps script metadata tolerant and raw-document friendly, while asset views are normalized and strict: character looks, storyboard grids, first frames, shot videos, edit renders, final videos, diagnostics, and explicit selected assets from closeout files.
...
- Workflow closeout files should be added by the AI video skills when available: `production/current-selection.json` for selected first frames/videos, `editing/current-edit.json` for the selected final/edit render, and `cost-summary.json` for per-project cost. If a selection file is absent, the Gear simply shows no selected marker.
