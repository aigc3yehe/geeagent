    ...
    private var filterBar: some View {
        HStack(spacing: 10) {
            ...
            TextField("Search name, tags, notes", text: $store.filter.searchText)
                ...
                .frame(height: MediaLibraryControlMetrics.height)
...
private enum MediaLibraryControlMetrics {
...
private struct EaglePillButtonStyle: ButtonStyle {
    ...
    func makeBody(configuration: Configuration) -> some View {
        configuration.label
            ...
            .padding(.horizontal, 12)
            .frame(height: MediaLibraryControlMetrics.height)
            .background(backgroundColor(isPressed: configuration.isPressed), in: RoundedRectangle(cornerRadius: MediaLibraryControlMetrics.radius, style: .continuous))
            .overlay(
                RoundedRectangle(cornerRadius: MediaLibraryControlMetrics.radius, style: .continuous)
...
private struct EagleIconButtonStyle: ButtonStyle {
    ...
    func makeBody(configuration: Configuration) -> some View {
        let size = compact ? MediaLibraryControlMetrics.compactHeight : MediaLibraryControlMetrics.height
        let radius = compact ? MediaLibraryControlMetrics.compactRadius : MediaLibraryControlMetrics.radius

        configuration.label
            .font(.system(size: compact ? 11 : 13, weight: .semibold))
            .foregroundStyle(isActive ? Color.white : Color.white.opacity(0.82))
            .frame(width: size, height: size)
            .background(backgroundColor(isPressed: configuration.isPressed), in: RoundedRectangle(cornerRadius: radius, style: .continuous))
            .overlay(
            ...
            ForEach(options, id: \.0) { option in
                ...
                Button {
                    ...
                    Label(option.1, systemImage: option.2)
                        ...
                        .frame(width: 74, height: MediaLibraryControlMetrics.height)
                        .lineLimit(1)
                        .fixedSize(horizontal: false, vertical: true)
                        .background(isSelected ? EaglePalette.accent : EaglePalette.control, in: RoundedRectangle(cornerRadius: MediaLibraryControlMetrics.radius, style: .continuous))
                }
                .buttonStyle(.plain)
            }
        }
...
private struct MediaLibraryFloatingToolbar: View {
    ...
    var body: some View {
        ...
        .padding(.horizontal, 14)
        .padding(.vertical, 10)
        .background(EaglePalette.panel.opacity(0.82), in: RoundedRectangle(cornerRadius: 12, style: .continuous))
        .overlay(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
    ...
    var body: some View {
        ZStack(alignment: .bottomLeading) {
            ...
            HStack(alignment: .top, spacing: 6) {
                ...
                .padding(.vertical, 4)
                .background(Color.black.opacity(0.46), in: RoundedRectangle(cornerRadius: 6, style: .continuous))
