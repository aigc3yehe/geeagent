...
struct HomeView: View {
    ...
    @State private var isTaskLauncherHovering = false
    ...
    private func companionCanvas(in size: CGSize) -> some View {
        ...
        ZStack {
            ...
            VStack(alignment: .trailing, spacing: metrics.bottomStackSpacing) {
                taskQueueLauncher(width: metrics.taskLauncherWidth, previewLimit: metrics.taskPreviewCount)
    ...
    private var homeWidgets: [InstalledAppRecord] {
        store.installedApps.filter {
            $0.isGearPackage
                ...
                && GearRegistry.isEnabled(gearID: $0.id)
        }
    }
    ...
    private func taskQueueLauncher(width: CGFloat, previewLimit: Int) -> some View {
        let cardPadding = width < 320 ? CGFloat(12) : CGFloat(14)
        ...
        return Button {
            ...
            VStack(alignment: .leading, spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 10) {
                    ForEach(Array(activeTasks.prefix(previewLimit))) { task in
                        HStack(spacing: 10) {
                            Image(systemName: task.status == .completed ? "checkmark.circle.fill" : "circle")
                                .font(.system(size: 13, weight: .light))
                                .foregroundStyle(task.status == .completed ? .white.opacity(0.52) : .white.opacity(0.34))

                            Text(task.title)
                                .font(.geeBody(13))
                                .foregroundStyle(.white.opacity(task.status == .completed ? 0.42 : 0.88))
                                .strikethrough(task.status == .completed, color: .white.opacity(0.18))
                                .lineLimit(1)
                        }
                    }
                }
            ...
            .glassCard(
                cornerRadius: 10,
                darken: isTaskLauncherHovering ? 0.28 : 0.1,
                materialOpacity: isTaskLauncherHovering ? 0.78 : 0.42
            )
            .homeBorderGlow(
                cornerRadius: 10,
                glowRadius: 34,
            ...
            .animation(.easeInOut(duration: 0.2), value: isTaskLauncherHovering)
        }
        .frame(width: width, alignment: .trailing)
        .buttonStyle(.plain)
        .onHover { isTaskLauncherHovering = $0 }
            ...
            if let conversation = store.selectedConversation {
                ScrollView {
                    VStack(alignment: .leading, spacing: 10) {
                        ...
                        if store.isSendingMessage {
                            TransientAgentActivityCard(
                                label: "GeeAgent is preparing the next step…",
                                prominentBackground: true
                            )
                        }
                    }
    ...
    private var selectedConversationTitle: String {
        store.selectedConversation?.displayTitle ?? "No Active Conversation"
    }
    ...
    private var homeConversationSummaryText: String {
        guard let conversation = store.selectedConversation else {
            return ""
        }
...
private struct HomeCanvasMetrics {
    ...
    var bottomStackSpacing: CGFloat {
        compactHeight ? 10 : 14
    }
    ...
    var taskLauncherWidth: CGFloat {
        min(max(availableCardWidth * 0.74, compactLauncherLayout ? 240 : 280), min(500, availableCardWidth))
    }
    ...
    var taskPreviewCount: Int {
        if size.height < 680 { return 1 }
        return compactHeight ? 2 : 3
    }
}

private enum HomeDisplayModeSwitcherMetrics {
    static let topInset: CGFloat = 18
    static let activationWidth: CGFloat = 176
...
private struct HomeFocusOverlayMetrics {
    ...
    var chatPanelHeight: CGFloat {
        min(max(size.height - verticalInset * 2, 320), 680)
    }
