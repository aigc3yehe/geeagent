...
- ネイティブ macOS アプリに同梱されるスタンドアロン Rust runtime bridge バイナリ
- `TOML`：ルーティングとモジュール設定
...
- `apps/runtime-bridge`：ネイティブ macOS シェルが利用する Rust bridge バイナリ
- `apps/desktop-shell/src-tauri`：旧シェルパスから移行中の Rust runtime library
...
このコマンドは、ネイティブ macOS アプリと、同梱される Rust runtime bridge を一緒にビルドします。
