...
- `Tauri v2` runtime bridge components for shell/runtime integration
...
- `apps/desktop-shell/src-tauri`: Rust bridge/runtime integration layer
...
### Build and Launch the Workbench Shell

```bash
bash apps/macos-bridge/script/build_and_run.sh
```

This command builds the native macOS app together with the Rust runtime bridge used by the shell.
